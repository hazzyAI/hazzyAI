...
명확한 케이스는 앞 단계에서 저비용으로 처리하고, 판별이 어려운 케이스만 다음 단계로 전달해 정확도와 추론 속도를 동시에 개선하였습니다.
현재 결과 도출은 완료하였고, 과제 성능 시험 및 정량적 과제 준비중에 있습니다.
...
피싱 사이트 탐지를 위한 **4-Modality VectorDB 기반 RAG 시스템**을 설계 완료·개발 및 테스트 중입니다.

```
url_text  ─▶ BGE-M3 ─▶ VectorDB (url)  ─┐
html_text ─▶ BGE-M3 ─▶ VectorDB (html) ─┤
ocr_text  ─▶ BGE-M3 ─▶ VectorDB (ocr)  ─┼─▶ RAG 검색 ─▶ Llama 3.1 70B (QLoRA)
raw_image ─▶ SigLIP ─▶ VectorDB (image)─┘
※ 4개 모달리티 각각 독립 VectorDB(Qdrant) 구성 · ocr_text 는 raw_image → PaddleOCR 로 추출
...
- **목표**: 의료 접근성 문제 해결 — 증상을 입력받아 가능성 있는 질병을 제시하는 **대화형 진단 AI**
- **데이터**: 공개 데이터 부재 → 병원 사이트 직접 크롤링 + LLM 기반 데이터 증강으로 학습셋 구축
- **모델**: KT Mi:dm LLM 파인튜닝 · 증상 입력 → 질병 후보 제시 → 추가 질문으로 진단 세분화
- **성과**: 주제·성능 우수성을 인정받아 **3위 (IITP 원장상)** 수상
...
- **목표**: 축사 이미지 실시간 추론으로 개별 한우의 발정시기를 정확히 예측
- **실험**: YOLOv5 · Mask R-CNN · Swin Transformer 비교 후 **Swin Transformer 최종 채택**
- **성과**: **Weighted mAP 0.97434** 달성 → KSC 2023 학술발표로 연결
...
- **목표**: 항체–항원 간 거리 예측으로 신약개발 소요기간 단축
- **접근**: BERT 기반 단백질 언어모델(ESM-v2) vs BioPython 도메인 특성 추출 방식 비교 실험
- **성과**: **ESM-v2(RMSE 4.37) 대비 RMSE 2.00866 · 54% 개선** → AAiCON 2023 학술발표로 연결
...
- **제안**: DNN 기반 약물–표적 단백질 상호작용(DTI) 모델로 약물 청소율 추정 시스템
- **성과**: **특허 제10-2730477호** 등록으로 연결
...
- **목표**: 마스킹 처리된(개인정보 보호) 유전체 256×256 이미지로 알츠하이머 이진 분류
- **모델**: **사전학습 모델인 Xception 파인튜닝**으로 발병 분류 모델 구성
- **성과**: **기존 SOTA 대비 Accuracy 2%+ 개선** → 신테카바이오 인턴십으로 연결
<br>
...
> 피싱·악성 URL 탐지 AI 서비스 연구 및 개발. 누적 다운로드 50만+ 보안 앱(싹다잡아)에 피싱 탐지 ML 모델 최초 설계 및 Android 온디바이스 배포. AI 탐지 기능을 탑재한 Chrome 확장프로그램 단독 개발 및 배포. 국가 R&D 과제(딥테크 · 스케일업 TIPS) 2건 병행 수행중.
...
![FeatureSelection](https://img.shields.io/badge/Feature_Selection-1B998B?style=flat-square)
![경량화](https://img.shields.io/badge/모델_경량화·온디바이스-555555?style=flat-square)
...
![Swin-Transformer](https://img.shields.io/badge/Swin_Transformer-2D8CFF?style=flat-square)

**LLM / RAG**

![Llama](https://img.shields.io/badge/Llama_3.1_70B-7C3AED?style=flat-square)
![QLoRA](https://img.shields.io/badge/QLoRA-9D4EDD?style=flat-square)
...
![ServiceWorker](https://img.shields.io/badge/Service_Worker-FFB300?style=flat-square)
![ONNXRuntime](https://img.shields.io/badge/ONNX_Runtime-005CED?style=flat-square&logo=onnx&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)

**자격증**

![TOEIC](https://img.shields.io/badge/TOEIC_Speaking-IH-0052CC?style=flat-square)
![정보통신산업기사](https://img.shields.io/badge/정보통신산업기사-취득-2E7D32?style=flat-square)
![통신선로산업기사](https://img.shields.io/badge/통신선로산업기사-취득-2E7D32?style=flat-square)
